...
# -*- coding: utf-8 -*-
"""
The program that implements experimental protocol 2 in the paper.
...
# initialize seiral port with a default baudrate of 115200
import serial
...
if write_enable:
    with open("Results/protocol2 " + datetime.datetime.now().strftime("%b %d %H:%M:%S"), 'w') as file:
...
if write_video_enable:
    ...
    out = cv2.VideoWriter("Videos/protocol2.avi", cv2.VideoWriter_fourcc(*"MJPG"), frame_count / (end_time - start_time), (width, height))
    for frame in frames:
